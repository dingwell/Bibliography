...
#
# Note: this list is not complete and will probably need to be expanded
...
if [[ -z $INFILE ]]; then
  echo "Need to specify an inputfile!" 1>&2
  exit 1
elif [[ -z $OUTFILE ]]; then
  OUTFILE=bibliography_new.bib
elif [[ ! -f $INFILE ]]; then
  echo "The file '$INFILE' does not exist" 1>&2
  exit 1
fi

# Ensure that input file and output file are not the same
# (only necessary if the ouputfile exists)
if [[ -f $OUTFILE ]]; then
  # (if two files have the same device-id and inode -- they're the same file)
  dev_inode1=$(stat -c "%d:%i" "$INFILE")
  dev_inode2=$(stat -c "%d:%i" "$OUTFILE")
  if [[ $dev_inode1 == $dev_inode2 ]]; then
    echo "'$INFILE' and '$OUTFILE' are the same file!" 1>&2
    exit 1
  fi
fi
